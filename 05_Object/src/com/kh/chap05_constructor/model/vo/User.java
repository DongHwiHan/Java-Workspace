...
//		기본생성자(매개변수가 없는 생성자)
//		간지 객체를 생성(메모리영역에 공간확보)만을 위한 목적으로 할때 사용함.
//		기본생성자를 생략하는 경우 => 오류나지않음.
//		JVM이 클래스를 컴파일할때 기본생성자가 없는경우 항상 자동으로 만들어 주기때문.(매개변수있는 생성자가 없는경우)
//		단, 매개변수가 있는 생성자가 이미 클래스안에 작성되어 있는경우 반드시 기본생성자도 함께 장성해줘야한다.
...
//		생성자 내부에서 생성자끼리 중복되는 코드가 있는경우, this생성자를 통해 같은 클래스 내에있는 다른생성자
