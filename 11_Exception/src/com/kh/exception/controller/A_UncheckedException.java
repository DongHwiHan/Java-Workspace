...
//	- ArrayIndexOutOfBoundsException : 배열의 부적절한 인덱스로 접근할때 밸생하는 예외 : 6, -1
...
//	- ArithmeticException : 나누기 연산시 0 으로 나눌때 발생하는 예외
//	-....

//	=> 이러한 RuntimeException과 관련한 예외는 충분히 예상 가능한 상황이기때문에
//	예외 자체가 발생이 안되게끔 조건문으로 해결이 가능함.
//	다라서 굳이 예외처리를 할 필요가 없음.

	Scanner sc = new Scanner(System.in);

	public void method1() {

//		ArithmeticException : 나누기 연산시 0 으로 나눌 때 발생하는 예외.
...
//		사용자에게 두개의 정수값을 입력받아 나눗셈 연산 결과 출력
		...
		int num2 = sc.nextInt();
//		해결방법 1. 조건문으로 처리(애초에 예외자체가 발생되지 않게끔 if문으로 조건검사하기)
//		if(num1 != 0 && num2 != 0) {
...
//		int[] arr = new int[Math.abs(size)]; // 방법 2. 항상 양의 정수값이 들어갈수 있도록 abs메서드 사용
//		}
//		try {
//			int[] arr = new int [size];
//			System.out.println("100번의 인덱스 값 : "+arr[100]);
//			
...
//			System.out.println("100번의 인덱스 값 : "+arr[100]);
//			
//		}catch(RuntimeException d) { // 다형성을 적용하여 부모타입의 예외클래스 작성 가능.
//			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, 부적정한 인덱스로 접근한것 같습니다.");
//		} // 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지 알수ㅏㄱ없다 
		try {
			int[] arr = new int [size];
			System.out.println("100번의 인덱스 값 : "+arr[100]);
			
		}catch(NegativeArraySizeException d) { // 다형성을 적용하여 부모타입의 예외클래스 작성 가능.
			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, 부적정한 인덱스로 접근한것 같습니다.");
...
//		- 예외처리 구문으로도 해결 가능 => 예외가 발생했을때를 대비해서 그 떄 실행할 내용을 정의해 두는것.
//		
