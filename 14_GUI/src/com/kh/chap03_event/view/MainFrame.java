...
//		plusBtn 클릭시 => 두개의 정수값을 가져온 후 덧셈연상 결과를 result텍스트 상자에 출력
...
//		ActionListener : 인터페이스 이기때문에 곧바로 생성 불가.
//		단, 여기서 생성을하고 싶다면 "미완성된 메소드"를  완성시키면서 생성 => 익명클래스 방식
...
//		minusBtn 클릭 시 => 두개의 정수값을 가져온 후 뺄셈연산 결과를 result텍스트박스에 출력
		...
		minusBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int v1 = Integer.parseInt(num1.getText());
				int v2 = Integer.parseInt(num2.getText());
				result.setText(v1 - v2 + "");
			}
		});

		JLabel title = new JLabel("두 개의 정수를 입려하세요.", 0); // 0 : 가운데 정렬
