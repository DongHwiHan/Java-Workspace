...
//	
//	2. 같은 자료형끼리만 연산 가능.
//	=> 즉, 다른형끼리 연산을 수행하고 싶으면 한구문은 "형변환"을 해야함.
//	[표현법] 값 + (바꿀자료형) 값;
//	
//	형변환의 종류
...
//		2. int(4byte) -> long(8byte)
		...
		long l4 = 2000L; // L을 붙이지 않아도 문제가 없는 이유 : 자동형변환이 되기때문

//		3. 특이케이스 long(8byte) -> float(4byte)
//		정수가 실수로 담길떄에는 큰사이즈의 정수여도 작은사이즈의 실수변수에 대입 가능.
...
//		
//		char의 범위 0~65xxx : 각 문자마다 고유의 정수값이 정해져있다.따라서 쌍방향 변환이
//		가능하다. 음수값은 오류가 난다.
...
//			  유니코드표(0 ~ 65xxx, 영어, 숫자, 특수문자, 한글, 그외 언어)
		...
		char sum = 'A' + 10; // 65 문자와 정수사이에 연산이 가능.
...
//		연산결과가 실수(double)자료형이므로 int형에 대입이 불가능.
...
//				 자동형변환은 작은 바이트변수가 큰 바이트 변수로 형변환 되기 때문
...
//		해결방법3. 애초에 결과값을 double형 번수에 담기.(데이터 손실없이 정확한 값이 담김)
