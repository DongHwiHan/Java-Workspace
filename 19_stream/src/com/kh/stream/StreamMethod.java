...
//		3_1) mapxxx : 현재요소를 xxx로 대체하여 스트림 반환
		...
		arr.stream()
			.map(food -> {if(food.equals("하와이안피자")) {
				return food + "맛없다.";
			}else {
				return food + "맛있다.";
			}
			...
			.forEach(System.out :: println);
		String[] strArr = {"마라탕 훠궈 양꼬치 김밥","호호 호호"};
//		3_2) flatMapxxx : 특정요소가 여러개의 요소로 대체되는 스트림 반환.
		List<String> arr2 = Arrays.asList(strArr); // 문자열 컬렉션으로 만들기 ["마라탕 훠궈 양꼬치 김밥"]
