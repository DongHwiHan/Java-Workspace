...
//		}
		...
		for (int num : arr2) {
//			arr2라는 배열은 0부터 인덱스부터 순차적으로 접근하면서 int num 이라는 변수에 arr2의 
//			각 인덱스의 값을 저장함.
		...
		phones[1] = new Phone("갤럭시", "zplip4", "삼성", 1500000);
...
//		"해당핸드폰이 없습니다"
		...
		int c = 0;
		for (Phone phone : phones) {
			if (phone.getSeries().equals(name)) {
				c++;
				System.out.println(
						phone.getName() + " " + phone.getSeries() + " 핸드폰의 가격은 " + phone.getPrice() + "원 입니다.");
				break;
			}
		}
		if (c == 0) {
