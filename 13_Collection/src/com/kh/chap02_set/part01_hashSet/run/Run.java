...
//		hs1.add(50);
		...
		System.out.println(hs1); // 중복 저장 x , 저장 순서 유지 x // [안녕하세요, 여러분, 반갑습니다.]

//		크기 구하기 : size()
		System.out.println("hs1의 크기는 ? " + hs1.size()); // hs1의 크기는 ? 3
...
//		모든 값 삭제 : clear();
		...
		hs2.add(new Student("민경민", 20, 100));
		hs2.add(new Student("이경이", 21, 90));
		hs2.add(new Student("삼경삼", 22, 80));

		System.out.println(hs2); // [Student [name=민경민, age=20, score=100], Student [name=이경이, age=21, score=90],
									// Student [name=삼경삼, age=22, score=80]]

//		저장 순서 유지 x, 중복 저장 o -> 동익객체로 판단되지 않기 때문.
...
//		1. for문 사용 가능(단, for each문(향상된 for문)만 가능 => index로 접근이 불가하므로)
		...
		for (Student s : hs2) {
			System.out.println(s); // Student [name=삼경삼, age=22, score=80]
									// Student [name=이경이, age=21, score=90]
									// Student [name=민경민, age=20, score=100]
		}
...
//		2. ArrayList에 담아준 후 ArrayList 반복문 수행.
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i));// Student [name=삼경삼, age=22, score=80]
											// Student [name=이경이, age=21, score=90]
											// Student [name=민경민, age=20, score=100]
		}
...
//		3. HashSet클래스에 제공하는 Iterator반복자를 이용한 방법.
